...
print(np.shape(rho[0]))
print(np.shape(rho[1]))
print(np.shape(rho[2]))
print(np.shape(rho[3]))
print(np.shape(rho[4]))
print(np.shape(rho[5]))

print(np.shape(xn[0][1:,1:])[0])
print(np.shape(xn[2][1:,1:])[1]) 


print(np.transpose(np.ones(np.shape(yn[2][1:,1:])[1])))



def concatenate_domains(obj, coordinate): 
    ALL=np.array([])
    if coordinate==0  : 
        #domain 0 and 1
        all_1=np.concatenate((obj[0],obj[1]))
        print(np.shape(all_1))

        #domain 2 and wall 
        #wall above tube :
        wall1=np.zeros((250, np.shape(obj[2])[1]))
        all_2=np.concatenate((wall1, obj[2]))
        print(np.shape(all_2))
        #domain 4 and 5 
        all_3=np.concatenate((obj[4], obj[5]))

        ALL=np.concatenate((all_1, all_2), axis=1)
        ALL=np.concatenate((ALL, all_3), axis=1)
   

    if coordinate==1 : #x coord 

        #domain 0 and 1
        all_1=np.concatenate((obj[0][1:,1:],obj[1][1:,1:]))
        print(np.shape(all_1))

        #domain 2 and wall 
        #wall above tube :
        wall1=np.transpose(np.array([xn[0][1:,1]]*2))
        print(np.shape(wall1))
        print( np.shape(obj[2][1:,1:]))
        all_2=np.concatenate((wall1, obj[2][1:,1:]))
        print(np.shape(all_2))

        #domain 4 and 5 
        all_3=np.concatenate((obj[4][1:,1:], obj[5][1:,1:]))

        ALL=np.concatenate((all_1, all_2), axis=1)
        ALL=np.concatenate((ALL, all_3), axis=1)
   

    if coordinate==2 : #x coord 
        ...
        #domain 2 and wall 
        #wall above tube :
        wall1=yn[2][1:,1:]*np.ones(np.shape(xn[0][1:,])[0])
        all_2=np.concatenate((wall1, obj[2][1:,1:]))
        print(np.shape(all_2))
...
All_rho=concatenate_domains(rho, 0)
All_x=concatenate_domains(xn,1)
All_y=concatenate_domains(yn,1)
All_p=concatenate_domains(p, 0)
All_u=concatenate_domains(utot, 0)

print(np.shape(All_rho))
print(np.shape(All_y))
print(np.shape(All_x))

plt.pcolormesh(All_x, All_y, All_p, cmap='jet')




print(All_x[0:200,0:14])
